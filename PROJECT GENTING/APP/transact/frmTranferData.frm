...
Begin VB.Form frm_trans_tranfer_data 
   ...
   Begin VB.TextBox txt_company_name 
      ...
      Width           =   2655
   End
   Begin prj_genting.vbButton btnBrowse 
      Height          =   315
      Left            =   4470
      TabIndex        =   23
      Top             =   6600
...
Dim v_company_code, v_ptkp, v_ptkp_detail As String
...
'        Call exportDataLeave
        ...
        MsgBox "Data Berhasil Di Eksport dengan rincian : " & Chr(13) & _
            ...
            v_loan & Chr(13) & v_loan_detail & Chr(13) & v_ptkp_detail, vbInformation, headerMSG
        cmdRun.Enabled = True
...
Else ' Import Transfer Type
    ...
    If OptData(0) Then ' Master Employee Data
        ...
        If v_company_code <> TDBCombo_company.Text Then
            MsgBox "Invalid File For This Company, Please Check Your File", vbExclamation, headerMSG
            cmdRun.Enabled = True
            Exit Sub
        End If
        Call importDataEmployee
...
Private Function check_exist_new_ptkp(ptkpCode As String) As Boolean
Dim rs As New ADODB.Recordset
Dim str_sql As String
check_exist_new_ptkp = False

str_sql = "select count(ptkp_code) as rec_count from m_ptkp where ptkp_code = '" & _
            Replace$(Trim$(ptkpCode), Chr$(39), Chr$(96)) & "'"
rs.Open str_sql, CnG, adOpenStatic, adLockReadOnly

If rs.Fields("rec_count").Value > 0 Then
    check_exist_new_ptkp = True
    Exit Function
End If
End Function

Private Function check_exist_new_ptkp_detail(ptkpCode As String, seq As String) As Boolean
Dim rs As New ADODB.Recordset
Dim str_sql As String
check_exist_new_ptkp_detail = False

str_sql = "select count(ptkp_code) as rec_count from m_ptkp_detail where ptkp_code = '" & _
            Replace$(Trim$(ptkpCode), Chr$(39), Chr$(96)) & "' " & _
            "AND ptkp_number = '" & seq & "'"
rs.Open str_sql, CnG, adOpenStatic, adLockReadOnly

If rs.Fields("rec_count").Value > 0 Then
    check_exist_new_ptkp_detail = True
    Exit Function
End If
End Function
...
conLite.Execute "CREATE TABLE IF NOT EXISTS m_salary_standard (" & _
                        ...
                        "special_allowance DECIMAL(15,2) DEFAULT NULL,employee_code TEXT(30) NOT NULL," & _
        ...
        strsql = "INSERT INTO m_salary_standard " & _
                    ...
                    "meal_allowance,special_allowance,employee_code,driver_allowance," & _
...
'                & "ELSE b.company_code = '" & TDBCombo_company.Text & "' END"
         strsql = "SELECT a.* FROM m_salary_standard a join m_employee b on a.employee_code = b.employee_code " _
                & "WHERE CASE WHEN '" & Check2.Value & "' = 1 then b.company_code = '" & TDBCombo_company.Text & "' AND b.department_code = '" & TDBCombo_department.Text & "' " _
                & "ELSE b.company_code = '" & TDBCombo_company.Text & "' END"
...
Private Sub exportDataPTKP()
Dim nmFile As String
Dim cmd As cCommand
Dim aa, bb, i As Integer
Dim rss As New cRecordset

conLite.Execute "CREATE TABLE IF NOT EXISTS m_ptkp (" & _
                        "ptkp_code TEXT(20) NOT NULL,ptkp_name TEXT(100)," & _
                        "PRIMARY KEY(ptkp_code)); "
                        
        strsql = "INSERT INTO m_ptkp " & _
                    "(ptkp_code,ptkp_name) " & _
                    "Values " & _
                    "(?,?)"
                                       
        Set cmd = conLite.CreateCommand(strsql)
        
        strsql = "SELECT ptkp_code,ptkp_name " & _
                 "FROM m_ptkp"
        ...
        'If LynxGrid1.Rows > 0 Then
            conLite.BeginTrans
            conLite.Execute ("DELETE FROM m_ptkp")
            ...
            If Not rsAtt.EOF Then
                ...
                While Not rsAtt.EOF
                    ...
                    'Set rsLite = conLite.OpenRecordset(strSQl, True)
                    cmd.SetText 1, rsAtt!ptkp_code
                    cmd.SetText 2, rsAtt!ptkp_name
                    cmd.Execute
                'End If
'            Next
                rsAtt.MoveNext
    ...
    Label2.Visible = False
    'MsgBox "Successfully Data Transferred. Total data transfered = " & ProgressBar1.Value & " !"
    v_ptkp = "Total Data PTKP = " & ProgressBar1.Value & " !"
    Exit Sub
End Sub

Private Sub exportDataPTKP_Detail()
Dim nmFile As String
Dim cmd As cCommand
Dim aa, bb, i As Integer
Dim rss As New cRecordset

conLite.Execute "CREATE TABLE IF NOT EXISTS m_ptkp_detail (" & _
                        "ptkp_code TEXT(20) NOT NULL,ptkp_number INTEGER NOT NULL," & _
                        "ptkp_name TEXT(50) DEFAULT NULL,ptkp_value DECIMAL(15,2) DEFAULT NULL, description TEXT(100) DEFAULT NULL," & _
                        "PRIMARY KEY(ptkp_code,ptkp_number)); "
                        
        strsql = "INSERT INTO m_ptkp_detail " & _
                    "(ptkp_code,ptkp_number,ptkp_name,ptkp_value,description) " & _
                    "Values " & _
                    "(?,?,?,?,?)"
                                       
        Set cmd = conLite.CreateCommand(strsql)
        
        strsql = "SELECT ptkp_code,ptkp_number,ptkp_name,ptkp_value,description " & _
                 "FROM m_ptkp_detail"
        ...
        'If LynxGrid1.Rows > 0 Then
            conLite.BeginTrans
            conLite.Execute ("DELETE FROM m_ptkp_detail")
            ...
            If Not rsAtt.EOF Then
                ...
                While Not rsAtt.EOF
                    ...
                    'Set rsLite = conLite.OpenRecordset(strSQl, True)
                    cmd.SetText 1, rsAtt!ptkp_code
                    cmd.SetInt32 2, rsAtt!ptkp_number
                    cmd.SetText 3, rsAtt!ptkp_name
                    cmd.SetDouble 4, rsAtt!ptkp_value
                    cmd.SetText 5, IIf(IsNull(rsAtt!Description), "", rsAtt!Description)
                    cmd.Execute
                'End If
'            Next
    ...
    Label2.Visible = False
    'MsgBox "Successfully Data Transferred. Total data transfered = " & ProgressBar1.Value & " !"
    v_ptkp_detail = "Total Data PTKP = " & ProgressBar1.Value & " !"
    Exit Sub
End Sub
...
'        Label2.Caption = "Total Data Transfered = " & ProgressBar1.Value & " / " & ProgressBar1.Max
        ...
        Else
            strsql = "UPDATE m_division SET company_code = '" & rss!COMPANY_CODE & "'," _
                ...
                & "division_code = '" & Replace(rss!division_code, "'", "''") & "'"
            CnG.Execute strsql
        End If
        rss.MoveNext
    Next
...
Dim rss As New cRecordset
    ...
    If Not SelectTable("m_ptkp") Then
        MsgBox "Cannot find data on selected file.. Please check your file again !"
        Exit Sub
    Else
        strsql = "SELECT * FROM m_ptkp"
        rss.OpenRecordset strsql, conLite, True
        Set rsLite = conLite.OpenRecordset(strsql, True)
...
'            End If
    ...
    If rs.State = 1 Then rs.Close
    rs.Open "select * from m_ptkp", CnG, adOpenKeyset, adLockOptimistic
...
'        Label2.Caption = "Total Data Transfered = " & ProgressBar1.Value & " / " & ProgressBar1.Max
        ...
        If Not check_exist_new_ptkp(rss!ptkp_code) Then
            i = i + 1
                    
            With rs
                .AddNew
                '-----------------------
                .Fields("ptkp_code").Value = rss!ptkp_code
                .Fields("ptkp_name").Value = rss!ptkp_name
                '-----------------------
                .Update
            End With
        Else
            strsql = "UPDATE m_ptkp SET ptkp_code = '" & rss!ptkp_code & "', " _
                & "ptkp_name = '" & rss!ptkp_name & "' " _
                & "WHERE ptkp_code = '" & rss!ptkp_code & "'"
            CnG.Execute strsql
        End If
        rss.MoveNext
    Next
...
'    Label2.Visible = False
    ProgressBar1.Visible = False
    'MsgBox "Successfully Data Transferred. Total data transfered = " & i & " !"
    v_ptkp = "Total Data PTKP = " & i & " !"
    Exit Sub
End Sub

Private Sub importDataPTKP_Detail()
Dim nmFile As String
Dim cmd As cCommand
Dim aa, bb, i As Integer
Dim rss As New cRecordset
    ...
    If Not SelectTable("m_ptkp_detail") Then
        MsgBox "Cannot find data on selected file.. Please check your file again !"
        Exit Sub
    Else
        strsql = "SELECT * FROM m_ptkp_detail"
        rss.OpenRecordset strsql, conLite, True
        Set rsLite = conLite.OpenRecordset(strsql, True)
...
'            End If
    ...
    If rs.State = 1 Then rs.Close
    rs.Open "select * from m_ptkp_detail", CnG, adOpenKeyset, adLockOptimistic
...
'        Label2.Caption = "Total Data Transfered = " & ProgressBar1.Value & " / " & ProgressBar1.Max
        ...
        If Not check_exist_new_ptkp_detail(rss!ptkp_code, rss!ptkp_number) Then
            i = i + 1
            ...
            With rs
                ...
                '-----------------------
                .Fields("ptkp_code").Value = rss!ptkp_code
                .Fields("ptkp_number").Value = rss!ptkp_number
                .Fields("ptkp_name").Value = rss!ptkp_name
                .Fields("ptkp_value").Value = rss!ptkp_value
                .Fields("description").Value = rss!Description
                '-----------------------
                .Update
            End With
        Else
            strsql = "UPDATE m_ptkp_detail SET ptkp_code = '" & rss!ptkp_code & "'," _
                & "ptkp_number = '" & rss!ptkp_number & "'," _
                & "ptkp_name = '" & rss!ptkp_name & "'," _
                & "ptkp_value = '" & rss!ptkp_value & "'," _
                & "description = '" & rss!Description & "' " _
                & "WHERE ptkp_code = '" & rss!ptkp_code & "' AND " _
                & "ptkp_number = '" & rss!ptkp_number & "'"
            CnG.Execute strsql
        End If
        rss.MoveNext
    Next
...
'    Label2.Visible = False
    ProgressBar1.Visible = False
    'MsgBox "Successfully Data Transferred. Total data transfered = " & i & " !"
    v_ptkp_detail = "Total Data PTKP = " & i & " !"
    Exit Sub
End Sub
