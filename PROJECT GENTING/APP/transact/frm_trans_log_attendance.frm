...
'                rsabsen!ip_address, rsabsen!enrollnumber, "", "STF", "P")
'            rsabsen.MoveNext
        Wend
    End If
...
TDBGrid1.Enabled = False: fra_button_control.Enabled = False
...
Else
    ...
    '    If FG_AUTO_DELETE_LOG = True Then
...
Public Sub delete_log_action_hkai()
    If rs_bound.State = 1 Then rs_bound.Close
    rs_bound.Open "select * from h_log_attendance where att_number = -1", CnG, adOpenKeyset, adLockOptimistic
    ...
    If Not connect_hkai Then
        If Not public_int_caller = 1 Then MsgBox "Error connecting to source device...", vbCritical, headerMSG
    Else
        If clear_all_log_hkai = False Then
            If Not public_int_caller = 1 Then MsgBox "Error deleting log...", vbCritical, headerMSG
        End If
        Call disconnect_hkai
    End If
...
Private Function disconnect_hkai() As Boolean
    If cn_fp Then
        If JT201A1.Disable(1) Then JT201A1.disconnect
        cn_fp = False
    End If
End Function
