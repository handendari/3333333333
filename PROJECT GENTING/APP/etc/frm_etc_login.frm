...
Begin VB.Form frm_etc_login 
   ...
   Begin VB.Label LblHeader 
      Alignment       =   2  'Center
      BackStyle       =   0  'Transparent
      Caption         =   "SSD ATTENDANCE && PAYROLL 2.0"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   14.25
         Charset         =   0
...
str_sql = "select a.user_code,a.user_name,a.user_pass,a.company_code," & _
                "a.flag_company_access,b.level_code, a.user_level,now() tglserver,b.department_code,a.flag_user,b.employee_name " & _
        "from m_user a left join m_employee b on a.employee_code = b.employee_code " & _
        "where user_name = '" & Replace(Trim(txtNama), "'", "''") & "' " & _
        "and user_pass = '" & Replace(RC4DeCryptASC(txtPassword.Text, pEncryptionPassword), "'", "") & "'"
...
'    End If
    ...
    If rs.RecordCount = 1 Then
        If Not Trim(rs!USER_NAME) = Trim(txtNama) Or _
            Not Trim(rs!USER_PASS) = RC4DeCryptASC(Replace(Trim(txtPassword.Text), Chr(39), Chr(96)), pEncryptionPassword) Then
