...
Begin VB.Form frm_mst_ptkp 
   ...
   Begin VB.Frame fra_entry1 
      ...
      End
   End
   Begin VB.TextBox txt_ptkp_name_type 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000B&
      Height          =   315
      Left            =   3030
   ...
   Begin TrueOleDBList60.TDBCombo TDBCombo_ptkp 
      Height          =   375
      Left            =   1230
      OleObjectBlob   =   "frm_mst_ptkp.frx":3770
      TabIndex        =   21
      Top             =   720
      Width           =   1695
   End
   ...
   End
End
Attribute VB_Name = "frm_mst_ptkp"
...
str_sql = "select count(ptkp_number) as rec_count from m_ptkp_detail where ptkp_number = " _
...
If Not txt_ptkp_number = Adodc1.Recordset.Fields("ptkp_number").Value And _
...
i = MsgBox("Are you sure want to delete data '" _
    & txt_ptkp_name_type.Text & "' ?", vbYesNo + vbQuestion, headerMSG)
...
i = MsgBox("Are you sure want to delete data '" _
    & TDBGrid1.Columns("ptkp_name").Value & "' ?", vbYesNo + vbQuestion, headerMSG)
If Not i = vbYes Then Exit Sub

CnG.BeginTrans
CnG.Execute "delete from m_ptkp_detail where ptkp_number = " _
    & "'" & TDBGrid1.Columns("ptkp_number").Value & "' AND ptkp_code = '" & TDBCombo_ptkp.Text & "'"
...
rsBound.Open "select * from m_ptkp_detail where ptkp_number = " _
& "'" & Adodc1.Recordset.Fields("ptkp_number").Value & "' AND ptkp_code = '" & TDBCombo_ptkp.Text & "'", CnG, adOpenKeyset, adLockOptimistic
...
If rsBound.State = 1 Then rsBound.Close
rsBound.Open "select * from m_ptkp_detail where ptkp_code = 'άφ' AND ptkp_number = 'άφ'", CnG, adOpenKeyset, adLockOptimistic
...
Else
    strsql = "INSERT INTO m_ptkp(ptkp_code,ptkp_name) " _
...
With rsBound
    ...
    .Fields("ptkp_code").Value = TDBCombo_ptkp.Text
    .Fields("ptkp_number").Value = Int(txt_ptkp_number)
    '-----------------------------------------------------------------------------
    .Fields("ptkp_name").Value = Trim(txt_ptkp_name)
    .Fields("ptkp_value").Value = Val(DropAllComma(txt_ptkp_value))
...
For Each Ctr In Me
    If TypeOf Ctr Is TextBox Or TypeOf Ctr Is TDBText Then
        If Not LCase(Ctr.name) = "txt_ptkp_name_type" Then Ctr.Text = ""
...
Adodc1.RecordSource = "select * from m_ptkp_detail where ptkp_code = '" & TDBCombo_ptkp.Text & "' order by ptkp_code"
Adodc1.Refresh
...
If TDBCombo_ptkp.ApproxCount > 0 Then
    ...
    txt_ptkp_name_type = TDBCombo_ptkp.Columns("ptkp_name").Value
