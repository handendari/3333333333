...
Begin VB.Form frm_mst_user 
   ...
   Begin VB.Frame fraPasteDtl 
      ...
      Visible         =   0   'False
      Width           =   5175
      Begin VB.CommandButton cmd_paste_cancel 
         Caption         =   "&Cancel"
         Height          =   645
         Left            =   3780
         Picture         =   "frm_mst_user.frx":0B14
         Style           =   1  'Graphical
         TabIndex        =   59
         Top             =   1290
         Width           =   975
      End
      Begin VB.CommandButton cmd_paste_ok 
         Caption         =   "&OK"
         Height          =   645
         Left            =   2760
         Picture         =   "frm_mst_user.frx":109E
         Style           =   1  'Graphical
         TabIndex        =   58
         Top             =   1290
         Width           =   975
   ...
   Begin VB.Frame fraCopyDtl 
      ...
      Visible         =   0   'False
      Width           =   5175
      Begin VB.CommandButton cmd_copy_cancel 
         Caption         =   "&Cancel"
         Height          =   645
         Left            =   3810
         Picture         =   "frm_mst_user.frx":1628
         Style           =   1  'Graphical
         TabIndex        =   61
         Top             =   1290
         Width           =   975
      End
      Begin VB.CommandButton cmd_copy_ok 
         Caption         =   "&OK"
         Height          =   645
         Left            =   2790
         Picture         =   "frm_mst_user.frx":1BB2
         Style           =   1  'Graphical
         TabIndex        =   60
         Top             =   1290
         Width           =   975
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frm_mst_user.frx":213C
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "SSTabUser"
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "USER"
      TabPicture(1)   =   "frm_mst_user.frx":2158
      ...
      Begin TabDlg.SSTab SSTabUser 
         ...
         TabPicture(0)   =   "frm_mst_user.frx":2174
      ...
      Begin VB.Frame fraAdmin 
         ...
         Top             =   420
         Width           =   10905
         Begin VB.CommandButton cmdPasteDtl 
            Caption         =   "&Paste"
            Height          =   555
            Left            =   9900
            Picture         =   "frm_mst_user.frx":37B8
            Style           =   1  'Graphical
            TabIndex        =   57
            Top             =   7950
            Width           =   855
         End
         Begin VB.CommandButton cmdCopyDtl 
            Caption         =   "&Copy"
            Height          =   555
            Left            =   8940
            Picture         =   "frm_mst_user.frx":3D42
            Style           =   1  'Graphical
            TabIndex        =   56
            Top             =   7950
            Width           =   855
         ...
         Begin TrueOleDBList60.TDBCombo TDBCombo_company 
            Height          =   375
            Left            =   1440
            OleObjectBlob   =   "frm_mst_user.frx":6992
            TabIndex        =   42
            Top             =   270
            Width           =   1695
         End
         Begin TrueOleDBList60.TDBCombo TDBCombo_userlevel 
            Height          =   375
            Left            =   1440
            OleObjectBlob   =   "frm_mst_user.frx":88F8
            TabIndex        =   43
            Top             =   690
            Width           =   1695
         End
...
Private Sub edit_data()
    ...
    If SSTab1.Tab = 0 Then
        ...
        If cmdEditTop.Caption = "&Edit" Then
            ...
            txt_pwd_super_user = RC4DeCryptASC(Replace(Trim(TDBGrid_SuperUser.Columns("user_pass").Value), Chr(39), Chr(96)), pEncryptionPassword)
    ...
    Else
        ...
        If cmdEditUser.Caption = "&Edit" Then
            ...
            txt_PasswordUser = RC4DeCryptASC(Replace(Trim(TDBGrid_User.Columns("user_pass").Value), Chr(39), Chr(96)), pEncryptionPassword)
...
Dim str_user_code As String, int_level As Integer
    ...
    int_level = IIf(TDBGrid_User.Columns("level_code").Value = "", 3, TDBGrid_User.Columns("level_code").Value)
...
'    CnG.Execute SQL
    ...
    SQL = "Insert into t_user (level_code, sub_menu_code, sub_menu_name, menu_code, menu_name, " & _
            ...
            "Select '" & str_user_code & "', sub_menu_code, sub_menu_name, menu_code, menu_name, form_name, form_title, " & _
                "1,1,1,1,1,1 " & _
            "from m_sub_menu where  " & _
                "ifnull(form_name,'')<>'' and sub_menu_code not in " & _
                "(select sub_menu_code from t_user where level_code = '" & str_user_code & "')"
    CnG.Execute SQL
    ...
    If SSTab1.Tab = 0 Then
        If fra_entry_super_user.Caption = "Entry" Then
            ...
            SQL = "insert into m_user (user_code, user_name, user_pass, user_level,full_name) " _
                ...
                & "','" & RC4DeCryptASC(Replace(Trim(txt_pwd_super_user.Text), Chr(39), Chr(96)), pEncryptionPassword) & "', " _
...
'            Call EnabledOptionTop(True)
        ...
        ElseIf fra_entry_super_user.Caption = "Edit" Then
            ...
            SQL = "update m_user set user_name = '" & Replace$(Trim$(txt_nama_super_user), Chr(39), Chr(96)) & _
                    ...
                    "', user_pass = '" & RC4DeCryptASC(Replace(Trim(txt_pwd_super_user.Text), Chr(39), Chr(96)), pEncryptionPassword) & _
    ...
    Else
        ...
        If fra_entry_user.Caption = "Entry" Then
            ...
            SQL = "INSERT INTO m_user(user_code,user_name,user_pass,user_level,company_code," & _
                    "employee_code,flag_user) " & _
                  "VALUES( " & _
                    "'" & vTot & "','" & Trim(txt_NamaUser) & "'," & _
                    "'" & Replace(RC4DeCryptASC(Trim(txt_PasswordUser.Text), pEncryptionPassword), "'", "''") & "'," & _
                    "0,'" & TDBCombo_company.Columns("company_code").Value & "'," & _
                    "'" & txtkd_employee.Text & "','" & chk_user_for.Value & "')"
            CnG.Execute SQL
...
Public Sub load_data_userlevel()
    ...
    If LOGIN_LEVEL = 100 Then
        SQL = "select code, name from m_akses_level_group order by code"
    Else
        SQL = "select code, name from m_akses_level_group where " & _
                "(code = ANY (SELECT access_level_code FROM t_user_access_level WHERE level_code = '" & LOGIN_CODE & "' AND allow_access <> 0)) " & _
                "order by code"
    End If
