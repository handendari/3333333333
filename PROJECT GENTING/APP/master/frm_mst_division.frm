...
CnG.Execute "delete from m_division where division_code = '" _
    & Replace(TDBGrid1.Columns("division_code").Value, "'", "''") & "'"
CnG.CommitTrans
...
& Replace(Adodc1.Recordset.Fields("division_code").Value, "'", "''") & "'", CnG, adOpenKeyset, adLockOptimistic
...
With rsBound
    ...
    .Fields("department_code").Value = TDBCombo_department.Columns("department_code").Value
    .Fields("department_name").Value = TDBCombo_department.Columns("department_name").Value
    .Fields("division_name").Value = Replace(Trim(txt_division_name), "'", "''")
    .Fields("description").Value = Trim(txt_description)
