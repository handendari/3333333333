...
Begin VB.Form frm_mst_bank 
   ...
   Begin VB.Frame fra_entry 
      ...
      Begin VB.ComboBox cbo_shiftable 
         Height          =   315
         ItemData        =   "frm_mst_bank.frx":000C
         Left            =   120
         List            =   "frm_mst_bank.frx":0016
         TabIndex        =   18
         Text            =   "..."
         Top             =   2400
         Visible         =   0   'False
...
If Not txt_bank_code = Adodc1.Recordset.Fields("bank_code").Value And _
...
If Trim(txt_bank_code) = "" Then
    MsgBox "Bank Code is empty!", vbOKOnly + vbInformation, headerMSG
...
If Trim(txt_bank_name) = "" Then
    MsgBox "Bank Name is empty!", vbOKOnly + vbInformation, headerMSG
...
CnG.BeginTrans
CnG.Execute "delete from m_bank where bank_code_code = '" _
...
rsBound.Open "select * from m_bank where bank_code = '" _
& Adodc1.Recordset.Fields("bank_code").Value & "'", CnG, adOpenKeyset, adLockOptimistic
...
If rsBound.State = 1 Then rsBound.Close
rsBound.Open "select * from m_bank where bank_code = 'άφ'", CnG, adOpenKeyset, adLockOptimistic
...
CnG.BeginTrans
strsql = "INSERT INTO m_bank(bank_code,bank_name,description) " _
        & "VALUES " _
...
'    .AddNew
'
'    .Fields("title_code").Value = Trim(txt_bank_code)
'    '-----------------------------------------------------------------------------
'    .Fields("default_shiftable").Value = cbo_shiftable.ListIndex
'    .Fields("title_name").Value = Trim(txt_bank_name)
...
CnG.BeginTrans
strsql = "UPDATE m_bank SET bank_code = '" & Trim(txt_bank_code.Text) & "'," _
...
Adodc1.RecordSource = "select * from m_bank order by bank_code"
Adodc1.Refresh
