...
Begin VB.Form frm_trans_log_attendance 
   ...
   Begin VB.Frame fra_button_control 
      ...
      Begin VB.CommandButton CmdExit 
         ...
         Height          =   645
         Left            =   2280
         Picture         =   "frm_trans_log_attendance.frx":05AE
         Style           =   1  'Graphical
         TabIndex        =   2
         Top             =   360
         Width           =   975
      ...
      Begin VB.CommandButton cmd_download 
         ...
         Height          =   645
         Left            =   1200
         Picture         =   "frm_trans_log_attendance.frx":0B38
         Style           =   1  'Graphical
         TabIndex        =   1
         Top             =   360
         Width           =   975
   ...
   Begin TrueOleDBList60.TDBCombo TDBCombo_company 
      Height          =   375
      Left            =   1200
      OleObjectBlob   =   "frm_trans_log_attendance.frx":10C2
      TabIndex        =   8
      Top             =   240
      Width           =   1695
   End
...
Dim vTglAkhir As String
        ...
        While CZKEM1.GetGeneralLogDataStr(vMachineNumber, dwEnrollNumber, dwVerifyMode, dwInOutMode, timeStr)
            ...
            SQL = "SELECT * FROM h_log_attendance " & _
                  "WHERE att_date = '" & Format(str_date, "yyyy-MM-dd HH:mm:ss") & "' " & _
                    "AND enrollnumber = '" & Val(dwEnrollNumber) & "'"
            rs7.Open SQL, CnG, adOpenForwardOnly, adLockReadOnly
            
            If rs7.RecordCount = 0 Then
                SQL = "INSERT INTO h_log_attendance(att_date, enrollnumber, " & _
                        "verifymode, flag_io, flag_attendance, entry_date) " & _
                      "VALUES( " & _
                        "'" & Format(str_date, "yyyy-MM-dd HH:mm:ss") & "', '" & Val(dwEnrollNumber) & "'," & _
                    ...
                    If CZKEM1.GetUserInfo(vMachineNumber, dwEnrollNumber, name, passWord, privilege, Enabled) Then
                        For iBackupNumber = 0 To 9
                            If CZKEM1.GetUserTmpStr(CLng(vMachineNumber), CStr(dwEnrollNumber), CLng(iBackupNumber), sTmpData, CLng(tmp_len)) Then
                                'Add to DB
                                SQL = "SELECT * FROM m_enroll WHERE EnrollNumber = '" & dwEnrollNumber & "' " & _
                                        "AND FingerNumber = '" & iBackupNumber & "'"
                                recFP.Open SQL, CnG, adOpenForwardOnly, adLockReadOnly
                                
                                If recFP.RecordCount = 0 Then
                                    SQL = "INSERT INTO m_enroll (EnrollNumber, EnrollName, EMachineNumber," & _
                                            "FingerNumber, Privilige, Password, FPData) " & _
                                          "VALUES (" & _
                                            "'" & dwEnrollNumber & "', '" & Replace(name, "'", "''") & "'," & _
            ...
            i = i + 1
            lbl_downloading.Caption = "Downloading...(" & i & " / " & j & ")"
        Wend
    ...
    If public_int_caller = 0 Then _
        MsgBox i & " are successfully downloaded...", vbInformation, headerMSG
        lbl_downloading.Caption = "Finish Downloading..."
        
        If vAutoDeleteLog <> 0 Then
            If clear_all_log = False Then
                If Not public_int_caller = 1 Then
                    lbl_downloading.Caption = "Error deleting log..."
                Else
                    lbl_downloading.Caption = "Delete log successfully..."
                End If
            End If
        End If
...
'            .AddNew
'
'            'CnG.Execute "insert into m_test (enrollnumber) values (" & dwEnrollNumber & ")"
'
...
Dim iMachineNumber As Long
    ...
    If alg_ver = "9" And (CZKEM1.IsTFTMachine(vMachineNumber) = False) Then 'Alg 9
        ...
        lbl_downloading.Caption = "Read Data Fingerprint..."
        Call disconnect
...
'    lbl_downloading.Caption = "Read Data Fingerprint..."
'    fra_downloading.Visible = True: fra_button_control.Enabled = False
...
'    If clear_all_log = False Then
'        MsgBox "Error deleting log...", vbCritical, headerMSG
'    End If
'End If
'
'Call disconnect
