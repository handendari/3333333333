...
rs.Open "select enrollnumber from m_enroll where ifnull(fpdata_created,0)=0 " _
...
'        Call frm_mst_enroll.download_enroll(running_rec)
        ...
        CnG.Execute "update m_enroll set fpdata_created=1 where enrollnumber=" & rs.Fields("enrollnumber").Value
        rs.MoveNext
    Wend
End If
