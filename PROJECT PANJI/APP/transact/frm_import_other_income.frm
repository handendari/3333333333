...
Begin VB.Form frm_import_other_income 
   Caption         =   "Import Data Other Income"
   ClientHeight    =   10785
   ClientLeft      =   120
   ClientTop       =   450
   ClientWidth     =   11760
...
Private Sub createGrid()
   With LynxGrid1
    .AddColumn "Trans. Date", 1200, lgAlignRightCenter, lgDate, "yyyy-MM-dd", , , , , , True
        ...
        .AddColumn "Flag Income", 1500, lgAlignCenterCenter, lgNumeric, , , , , , , True
...
'    DoEvents
    strWorksheet = "data_other_income"
        ...
        If .Rows > 0 Then
            ...
            For aa = 0 To .Rows - 1
                ...
                If rsnumber.RecordCount > 0 Then
                    ...
                    SQL = "DELETE FROM t_employee_income WHERE employee_code = '" & v_employee_code & "' " & _
                            "AND date(tgltrans) = '" & IIf(IsNull(.CellText(aa, 0)), "", Format(.CellText(aa, 0), "yyyy-MM-dd")) & "' " & _
                            "AND flag_other_income = '" & .CellValue(aa, 4) & "'"
                    CnG.Execute SQL
                    
                    Call BuatKode
                    
                    If .CellText(aa, 0) <> "" And .CellText(aa, 1) <> "" Then
                    SQL = "INSERT INTO t_employee_income (nomer,notrans," _
                        & "tgltrans,employee_code,jmlPotong,remark," _
                        & "userinput,tglinput,flag_other_income)" _
                        & "VALUES " _
                        & "('" & nomer & "','" & noTrans & "'," _
...
Dim rs2 As New ADODB.Recordset
    ...
    If IsNull(rs2!nomer) = False Then
        ...
        noTrans = LOGIN_CODE & "/OEI/" & bulan & "/" & Right(rs2!tahun, 2) & "/" & rs2!nomer
    Else
        nomer = "1"
        bulan = IIf(Len(month(Date)) = 1, "0" & month(Date), month(Date))
        noTrans = LOGIN_CODE & "/OEI/" & bulan & "/" & Right(year(Date), 2) & "/00001"
    End If
    rs2.Close
End Sub
