...
Begin VB.Form frm_rpt_summary_employee 
   ...
   Begin VB.Frame Frame3 
      ...
      Begin prj_panji.vbButton cmdExit 
         ...
         MCOL            =   12632256
         MPTR            =   1
         MICON           =   "frm_rpt_summary_employee.frx":2529
         PICN            =   "frm_rpt_summary_employee.frx":2545
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   2
         NGREY           =   0   'False
      ...
      Begin prj_panji.vbButton cmdPrint 
         ...
         MCOL            =   12632256
         MPTR            =   1
         MICON           =   "frm_rpt_summary_employee.frx":35D7
         PICN            =   "frm_rpt_summary_employee.frx":35F3
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   2
         NGREY           =   0   'False
   ...
   Begin TrueOleDBList60.TDBCombo TDBCombo_company 
      Height          =   375
      Left            =   1530
      OleObjectBlob   =   "frm_rpt_summary_employee.frx":4685
      TabIndex        =   3
      Top             =   750
      Width           =   1695
   End
   ...
   Begin VB.Image Image1 
      Height          =   585
      Left            =   0
      Picture         =   "frm_rpt_summary_employee.frx":65EB
      Stretch         =   -1  'True
      Top             =   0
      Width           =   14850
   End
...
'                   "a.place_of_birth,a.date_of_birth,a.address, ROUND(DATEDIFF(NOW(),date_birth)/365) umur," & _
...
'                "WHERE " & access & " " & _
'                   " AND CASE WHEN '" & tdbcombo_division.Text = "" & "' THEN a.company_code = '" & TDBCombo_company.Columns("company_code").Value & "' " & _
'                        "ELSE a.company_code = '" & TDBCombo_company.Columns("company_code").Value & "' AND a.department_code = '" & tdbcombo_division.Text & "' END " & _
'                   "ORDER BY employee_name ASC) aa " & _
'          "" & v_sex & " " & v_age & " " & v_working_age & ""
    SQL = "SELECT * FROM (SELECT a.company_code,b.company_name," & _
                   "a.nik,a.employee_name,a.title_code,e.title_name,a.marital_status," & _
                   ...
                   "ROUND(DATEDIFF(NOW(),start_working)/365) lama_bekerja " & _
                "FROM m_employee a LEFT JOIN m_company b ON a.company_code = b.company_code " & _
                "LEFT JOIN m_division d ON a.division_code = d.division_code AND d.company_code = b.company_code " & _
                "LEFT JOIN m_title e ON a.title_code = e.title_code " & _
                "WHERE a.flag_active <> 0 AND " & IIf(TDBCombo_division.Text <> "", "a.company_code = '" & TDBCombo_company.Columns("company_code").Value & "' AND a.division_code = '" & TDBCombo_division.Text & "'", "a.company_code = '" & TDBCombo_company.Columns("company_code").Value & "'") & " " & _
                    "" & access & " " & _
                "ORDER BY employee_name ASC) aa " & _
          "" & v_sex & " " & v_age & " " & v_working_age & ""
