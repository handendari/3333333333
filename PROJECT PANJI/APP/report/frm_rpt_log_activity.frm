...
Begin VB.Form frm_rpt_log_activity 
   ...
   Caption         =   "LAPORAN - CATATAN AKTIFITAS"
   ClientHeight    =   7365
   ClientLeft      =   -15
   ClientTop       =   240
   ClientWidth     =   12885
   ...
   Begin VB.Frame frmTombol 
      ...
      Begin prj_panji.vbButton cmd_delete_all 
         ...
         VALUE           =   0   'False
      End
      Begin prj_panji.vbButton cmdExport 
         Height          =   705
         Left            =   5010
         TabIndex        =   7
         Top             =   390
         ...
         MCOL            =   12632256
         MPTR            =   1
         MICON           =   "frm_rpt_log_activity.frx":37A0
         PICN            =   "frm_rpt_log_activity.frx":37BC
         UMCOL           =   -1  'True
         SOFT            =   0   'False
         PICPOS          =   2
         NGREY           =   0   'False
   ...
   Begin VB.Image Image1 
      Height          =   585
      Left            =   0
      Picture         =   "frm_rpt_log_activity.frx":484E
      Stretch         =   -1  'True
      Top             =   0
      Width           =   14850
   End
...
Dim i As Integer
    ...
    i = MsgBox("Apakah Yakin Akan Menghapus Semua Log Data?", vbYesNo + vbQuestion, headerMSG)
    If Not i = vbYes Then Exit Sub
...
Dim DataArray(1 To 1000000, 1 To 6) As Variant
Dim r As Integer
...
'        End If
    ...
    strsql = "select b.user_name,c.company_name,d.division_name,login_time,click_time,a.description " _
    ...
    Dim vRecord As String
        For v_loop = 1 To 6
        vRecord = IIf(v_loop = 1, rs!USER_NAME, IIf(v_loop = 2, IIf(IsNull(rs!company_name), "", rs!company_name), IIf(v_loop = 3, IIf(IsNull(rs!division_name), "", rs!division_name), _
                IIf(v_loop = 4, rs!login_time, IIf(v_loop = 5, rs!click_time, rs!Description)))))
        
        DataArray(r, v_loop) = vRecord
        Next v_loop
...
'    oSheet.Range("A1:AS11").Font.Bold = True
    ...
    oSheet.Range("A1:F1").Value = Array("NAMA USER", "PERUSAHAAN", "DIVISI", "WAKTU LOGIN", _
                                "WAKTU KLIK", "DESKRIPSI")
    ...
    MsgBox "Export Data Berhasil", vbInformation, headerMSG
...
If Err.Number = 1004 Then
    MsgBox "Data Gagal Diimport...", vbExclamation, headerMSG
    rs.Close
    Exit Sub
Else
...
Private Sub load_data_log()
    ...
    SQL = "select a.*,b.user_name,c.company_name,d.division_name " _
        & "from log_activitas a join m_user b on a.user_code = b.user_code " _
        & "LEFT JOIN m_company c on a.company_code = c.company_code " _
        & "LEFT JOIN m_division d on a.division_code = d.division_code AND a.company_code = d.company_code " _
        & "ORDER BY click_time DESC"
