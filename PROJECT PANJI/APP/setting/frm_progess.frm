...
Dim start_rec, running_rec, end_rec As Long
...
On Error GoTo Err
    ...
    If FG_DEVICE_TYPE = 0 Then
        a = frm_mst_enroll.get_all_user_info_tipeA
    Else
        a = frm_mst_enroll.get_all_user_info_tipeB
    End If
    
    If rs.State = 1 Then rs.Close
    rs.Open "select enrollnumber from m_enroll where ifnull(fpdata_created,0)=0 and ip_address = '" _
        & FG_IP_ADDRESS & "'" & " order by enrollnumber asc", CnG, adOpenKeyset, adLockOptimistic
    ...
    '        If FG_DEVICE_TYPE <> 1 Then Call frm_mst_enroll.download_enroll(running_rec)
            
            Call show_progrees
            
            CnG.Execute "update m_enroll set fpdata_created=1 where enrollnumber=" & Val(rs.Fields("enrollnumber").Value) _
