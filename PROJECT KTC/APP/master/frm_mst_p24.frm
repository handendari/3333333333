...
Begin VB.Form frm_mst_p24 
   ...
   Begin VB.Frame fra_entry 
      ...
      Top             =   2280
      Width           =   11295
      Begin VB.TextBox txt_p24_value 
         Appearance      =   0  'Flat
         Height          =   315
         Left            =   5280
         MaxLength       =   50
         ...
         Width           =   1695
      End
      Begin VB.CheckBox chk_flag_year_over 
         Height          =   255
         Left            =   5280
         TabIndex        =   4
         Top             =   1680
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
      ...
      Splits(0)._ColumnProps(16)=   "Column(3).Width=3228"
      Splits(0)._ColumnProps(17)=   "Column(3).DividerColor=0"
      Splits(0)._ColumnProps(18)=   "Column(3)._WidthInPix=3149"
...
str_sql = "select count(p24_number) as rec_count from m_p24 where p24_number = " & Trim(txt_p24_number)
...
If Not txt_p24_number = Adodc1.Recordset.Fields("p24_number").Value And _
...
i = MsgBox("Are you sure want to delete data '" _
    & TDBGrid1.Columns("p24_number").Value & "' ?", vbYesNo + vbQuestion, headerMSG)
If Not i = vbYes Then Exit Sub

CnG.BeginTrans
CnG.Execute "delete from m_p24 where p24_number = '" _
    & TDBGrid1.Columns("p24_number").Value & "'"
CnG.CommitTrans
...
With Adodc1.Recordset
    txt_p24_number = .Fields("p24_number").Value
    DTPicker_date_from = .Fields("date_from").Value
    DTPicker_date_to = .Fields("date_to").Value
    chk_flag_year_over.Value = .Fields("flag_year_over").Value
    txt_p24_value = .Fields("p24_value").Value
End With
End Sub

Private Sub cmdEdit_Click()
If rsBound.State = 1 Then rsBound.Close
rsBound.Open "select * from m_p24 where p24_number = " _
& Adodc1.Recordset.Fields("p24_number").Value, CnG, adOpenKeyset, adLockOptimistic
...
If rsBound.State = 1 Then rsBound.Close
rsBound.Open "select * from m_p24 where p24_number = 'άφ'", CnG, adOpenKeyset, adLockOptimistic
...
With rsBound
    ...
    .Fields("p24_number").Value = Val(txt_p24_number)
    '-----------------------------------------------------------------------------
    .Fields("date_from").Value = DTPicker_date_from
    .Fields("date_to").Value = DTPicker_date_to
    .Fields("flag_year_over").Value = IIf(chk_flag_year_over, 1, 0)
    .Fields("p24_value").Value = Val(txt_p24_value)
...
With rsBound
    ...
    .Fields("date_to").Value = DTPicker_date_to
    .Fields("flag_year_over").Value = IIf(chk_flag_year_over, 1, 0)
    .Fields("p24_value").Value = Val(DropAllComma(txt_p24_value))
...
Adodc1.RecordSource = "select * from m_p24 order by p24_number"
Adodc1.Refresh
