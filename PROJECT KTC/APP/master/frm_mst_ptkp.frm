...
str_sql = "select count(ptkp_number) as rec_count from m_ptkp where ptkp_number = " _
& Int(txt_ptkp_number)
...
CnG.Execute "delete from m_ptkp where ptkp_number = " _
    & TDBGrid1.Columns("ptkp_number").Value
CnG.CommitTrans
...
rsBound.Open "select * from m_ptkp where ptkp_number = " _
& Adodc1.Recordset.Fields("ptkp_number").Value, CnG, adOpenKeyset, adLockOptimistic
...
If rsBound.State = 1 Then rsBound.Close
rsBound.Open "select * from m_ptkp where ptkp_number = 'άφ'", CnG, adOpenKeyset, adLockOptimistic
...
Adodc1.RecordSource = "select * from m_ptkp order by ptkp_number"
Adodc1.Refresh
