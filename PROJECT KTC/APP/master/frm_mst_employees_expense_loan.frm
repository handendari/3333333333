...
Begin VB.Form frm_mst_salary_exp_employee_installment 
   ...
   Caption         =   "EMPLOYEE'S INSTALLMENT"
   ClientHeight    =   9090
   ClientLeft      =   -15
   ClientTop       =   240
   ClientWidth     =   14685
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid2 
      ...
      HeadLines       =   1
      FootLines       =   1
      Caption         =   "LIST OF INSTALLMENT DETAIL"
      MultipleLines   =   0
      CellTipsWidth   =   0
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
      ...
      Columns(11).DataField=   "loan_value"
...
frm_lookup_mst_employee.public_int_mode = 103
...
'    txt_item_type_name = .Fields("item_type_name").Value
    ...
    txt_loan_value = FormatNumber(DropAllComma(.Fields("loan_value").Value))
...
Adodc1.RecordSource = "select a.* from tm_loan a join m_employee b on a.employee_code = b.employee_code " _
    & "where a.company_code = '" _
    & TDBCombo_company.Columns("company_code").Value & "' " & access & " and a.flag_sign=-1 " _
    & "order by a.employee_code asc, a.salary_item_code asc, a.date desc"
Adodc1.Refresh
...
Private Sub txt_loan_value_GotFocus()
    txt_loan_value.SetFocus
    txt_loan_value.SelStart = 0
    txt_loan_value.SelLength = Len(txt_loan_value.Text)
End Sub
