...
Begin VB.Form frm_lookup_mst_employee 
   ...
   Begin VB.Frame frmTombol 
      ...
      Begin prj_tpc.vbButton CmdSave 
         ...
         VALUE           =   0   'False
      End
      Begin prj_tpc.vbButton cmdClose 
         Height          =   705
         Left            =   2100
         TabIndex        =   9
         Top             =   390
   ...
   Begin VB.Image Image1 
      Height          =   585
      Left            =   0
      Picture         =   "frm_lookup_mst_employee.frx":51B8
      Stretch         =   -1  'True
      Top             =   0
      Width           =   14850
   End
...
Private Function check_validate_exist_emp_group(ByVal str_company_code As String, _
ByVal int_group_number As Integer, ByVal str_employee_code As String, ByVal str_group_code As String) As Boolean
Dim rs As New ADODB.Recordset
Dim str_sql As String
    check_validate_exist_emp_group = False

    str_sql = "select count(*) as rec_count from td_emp_group where company_code='" & str_company_code _
                & "' and group_number = " & int_group_number & " and employee_code = '" & str_employee_code & "' " & _
                "AND group_code = '" & str_group_code & "'"
    rs.Open str_sql, CnG, adOpenStatic, adLockReadOnly

    If rs.Fields("rec_count").Value > 0 Then
        check_validate_exist_emp_group = True
        Exit Function
    End If
End Function
...
'On Error GoTo Err
    ...
    If public_int_mode = 0 Then
        ...
        If public_int_mode = 0 Then
            For Each item In SelBks
                ...
                SQL = "SELECT flag_rollable FROM m_shift_group WHERE group_code = '" & frm_mst_working_time.TDBCombo_group_wt(0).Text & "'"
                ...
                SQL = "SELECT date(start_date) start_date " & _
                      ...
                      "WHERE employee_code = '" & TDBGrid1.Columns("employee_code").CellText(item) & "' " & _
                        "AND group_code = '" & frm_mst_working_time.TDBCombo_group_wt(0).Text & "'"
                rs.Open SQL, CnG, adOpenForwardOnly, adLockReadOnly
                ...
                SQL = "SELECT shift_number FROM tm_shift WHERE group_code = '" & frm_mst_working_time.TDBCombo_group_wt(0).Text & "'"
                ...
                If v_rollable = 1 Then
                    ...
                    If rs.RecordCount > 0 Then
                        If Not rs.EOF Then
                            ...
                            While Not rs.EOF
                                ...
                                frm_mst_working_time.TDBCombo_group_wt(0).Text) = False Then
                                    
                                    CnG.Execute "insert into td_shift values " & _
                                        "('" & TDBGrid1.Columns("company_code").CellText(item) & "', " & _
                                        "'" & rs!shift_number & "', " & _
                                        "'" & TDBGrid1.Columns("employee_code").CellText(item) & "', " & _
                                        "'" & frm_mst_working_time.TDBCombo_group_wt(0).Text & "')"
                    ...
                    End If
                Else
                    If Format(DTPicker_start_date.Value, "yyyy-MM-dd") > v_start_date Then
                        If check_validate_exist_employee_wt_new(TDBGrid1.Columns("company_code").CellText(item), _
                        rs!shift_number, _
                        TDBGrid1.Columns("employee_code").CellText(item), _
                        frm_mst_working_time.TDBCombo_group_wt(0).Text) = False Then
                            ...
                            SQL = "UPDATE m_employee SET flag_shiftable = 0 " & _
                                    "WHERE employee_code = '" & TDBGrid1.Columns("employee_code").CellText(item) & "'"
                            CnG.Execute SQL
                        End If
                        ...
                        CnG.Execute "insert into td_shift2 values " & _
                            ...
                            "'" & frm_mst_working_time.TDBGrid_EmpWT.Columns("shift_number").Value & "', " & _
                            "'" & TDBGrid1.Columns("employee_code").CellText(item) & "', " & _
                            "'" & frm_mst_working_time.TDBCombo_group_wt(0).Text & "'," & _
                            "'" & frm_mst_working_time.TDBGrid_EmpWT.Columns("shift_code").Value & "'," & _
                            "'" & Format(DTPicker_start_date, "yyyy-MM-dd") & "')"
                    Else
                        MsgBox "Last Setting Date For Employee : " & TDBGrid1.Columns("employee_name").CellText(item) & _
                               " is Bigger Than This Setting Date!", vbExclamation, headerMSG
                        j = j + 1
                    End If
                End If
                rs.Close
        ...
        MsgBox i - j & " employee's data are successfully added", vbInformation, headerMSG
    ...
    ElseIf public_int_mode = 6 Then
        ...
        For Each item In SelBks
            ...
            If check_validate_exist_emp_group(TDBGrid1.Columns("company_code").CellText(item), _
            frm_mst_working_time.TDBGrid_EmpGroup.Columns("emp_group_number").Value, _
            TDBGrid1.Columns("employee_code").CellText(item), _
            frm_mst_working_time.TDBCombo_group_wt(1).Text) = False Then
                SQL = "insert into td_emp_group values " _
                    & "('" & TDBGrid1.Columns("company_code").CellText(item) & "'," _
                    & "'" & frm_mst_working_time.TDBGrid_EmpGroup.Columns("emp_group_number").Value & "'," _
                    & "'" & TDBGrid1.Columns("employee_code").CellText(item) & "'," _
                    & "'" & frm_mst_working_time.TDBCombo_group_wt(1).Text & "'," _
                    & "'" & frm_mst_working_time.TDBGrid_EmpGroup.Columns("emp_group_code").Value & "'," _
                    & "'" & Format(DTPicker_start_date.Value, "yyyy-MM-dd") & "')"
                CnG.Execute SQL
                
                SQL = "UPDATE m_employee SET flag_shiftable = 1 " & _
                        "WHERE employee_code = '" & TDBGrid1.Columns("employee_code").CellText(item) & "'"
                CnG.Execute SQL
            End If
        Next
        ...
        CnG.CommitTrans
        Call frm_mst_working_time.load_data_detail_emp_group
...
Private Sub load_data_employee()
    ...
    If public_int_mode = 0 Then
'        SQL = "SELECT a.*,b.department_name,c.division_name,d.title_name " & _
'                  "FROM m_employee a JOIN m_department b ON a.department_code = b.department_code " & _
...
'                & "(select employee_code from td_shift) order by employee_name"
    ...
    ElseIf public_int_mode = 82 Then
        If frm_mst_user.TDBCombo_company.Text <> "" And frm_mst_user.TDBCombo_division = "" And frm_mst_user.TDBCombo_section = "" Then
            SQL = "SELECT a.*,b.department_name,c.division_name,d.title_name " & _
                      "FROM m_employee a JOIN m_department b ON a.department_code = b.department_code " & _
                        "JOIN m_division c ON a.division_code = c.division_code " & _
                        "LEFT JOIN m_title d ON a.title_code = d.title_code " & _
                  "WHERE a.company_code = '" _
                    & TDBCombo_company.Columns("company_code").Value & "'"
        ElseIf frm_mst_user.TDBCombo_company.Text <> "" And frm_mst_user.TDBCombo_division <> "" And frm_mst_user.TDBCombo_section = "" Then
            SQL = "SELECT a.*,b.department_name,c.division_name,d.title_name " & _
                      "FROM m_employee a JOIN m_department b ON a.department_code = b.department_code " & _
                        "JOIN m_division c ON a.division_code = c.division_code " & _
                        "LEFT JOIN m_title d ON a.title_code = d.title_code " & _
                  "WHERE a.company_code = '" _
                    ...
                    & "a.department_code = '" & frm_mst_user.TDBCombo_division.Text & "' order by employee_name"
        Else
            SQL = "SELECT a.*,b.department_name,c.division_name,d.title_name " & _
                      "FROM m_employee a JOIN m_department b ON a.department_code = b.department_code " & _
                        "JOIN m_division c ON a.division_code = c.division_code " & _
                  ...
                  "WHERE a.company_code = '" _
                    & TDBCombo_company.Columns("company_code").Value & "' AND " _
                    & "a.department_code = '" & frm_mst_user.TDBCombo_section.Text & "' AND " _
                    & "a.division_code = '" & frm_mst_user.TDBCombo_section.Text & "' order by employee_name"
        End If
    Else
