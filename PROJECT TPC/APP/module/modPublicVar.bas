...
Public COMPANY_CODE, LOGIN_NAME, LOGIN_FULLNAME, LOGIN_PASS, LOGIN_CODE, DEPARTMENT_CODE, EMPLOYEE_NAME, DIVISION_CODE As String
...
Public CnG1              As ADODB.Connection
Public rs               As New ADODB.Recordset
Public rscari           As New ADODB.Recordset
Public rsList           As New ADODB.Recordset
Public rsPeriode        As New ADODB.Recordset

Public strConn As String, ServerDB As String, UserDB As String
Public nmDB As String, passDB As String, PortDB As String, TypeDB As Integer
...
Public x As Integer
Public vSubject As String
Public oClause As String

Public vFlagPeriode As Integer
Public vDayStart As Integer
Public vDayEnd As Integer

Public vModeLoad           As Integer 'Untuk menentukan Mode Load di List Manual Attendance
...
On Error Resume Next
    ...
    If GetInitEntry(3, "CONFIG", "A") <> "" Then
        TypeDB = DecryptINI(GetInitEntry(3, "CONFIG", "A"), pEncryptionPassword)
        
        If GetInitEntry(TypeDB, "CONFIG", "A") <> "" Then
            ServerDB = DecryptINI(GetInitEntry(TypeDB, "CONFIG", "A"), pEncryptionPassword)
            UserDB = DecryptINI(GetInitEntry(TypeDB, "CONFIG", "B"), pEncryptionPassword)
            passDB = DecryptINI(GetInitEntry(TypeDB, "CONFIG", "C"), pEncryptionPassword)
            nmDB = DecryptINI(GetInitEntry(TypeDB, "CONFIG", "D"), pEncryptionPassword)
            PortDB = DecryptINI(GetInitEntry(TypeDB, "CONFIG", "E"), pEncryptionPassword)
        End If
    End If
End Sub
