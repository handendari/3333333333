...
'Dim h As New HDSN
'
'rs1.Open "SELECT date_add(register_date,interval +2 week) as status FROM m_license " _
...
Public Sub set_company_mode_adodc(ByRef Adodc_company As Adodc, _
...
Dim vCompany As String
    ...
    If LOGIN_LEVEL = 100 Or COMPANY_ACCESS = 1 Then
        ...
        SQL = "SELECT company_code from m_pref_gen"
        rscari.Open SQL, CnG, adOpenForwardOnly, adLockReadOnly
        
        If rscari.RecordCount > 0 Then
            vCompany = rscari.Fields("company_code").Value
        Else
            vCompany = ""
        End If
        rscari.Close
...
Public Sub set_company_mode(ByRef rs As Recordset, _
                            ByRef TDBCombo_company As TDBCombo, _
                            ByRef txt_company_name As TextBox)
Dim vCompany As String
